...
def main():
    ...
    if args.method == "lora":
        adapter_bin = os.path.join(args.model_path, "adapter_model.bin")
        adapter_safe = os.path.join(args.model_path, "adapter_model.safetensors")
        if os.path.exists(adapter_safe):
            ...
            print(f"⚠️ Warning: No LoRA adapter weights found in {args.model_path}!")
    else:
